...
require "fast/prometheus/otlp/mapper"

describe Fast::Prometheus::OTLP::Mapper do
  let(:registry) { Fast::Prometheus::Registry.new }
  let(:start_time) { Time.now }
  let(:mapper) { Fast::Prometheus::OTLP::Mapper.new(start_time: start_time) }
  ...
  describe "#request" do
    ...
    describe "native_histogram" do
      ...
      it "fills gaps with zeros for sparse observations" do
        registry2 = Fast::Prometheus::Registry.new
        ...
        mapper2 = Fast::Prometheus::OTLP::Mapper.new
    ...
    describe "resource and scope" do
      ...
      it "has correct scope name and version" do
        ...
        expect(scope.name).to be(:==, "fast-prometheus")
        expect(scope.version).to be(:==, Fast::Prometheus::VERSION)
      end
