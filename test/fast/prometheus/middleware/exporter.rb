...
describe Fast::Prometheus::Middleware::Exporter do
  ...
  def middleware
    Fast::Prometheus::Middleware::Exporter.new(delegate, registry: registry)
  end
  ...
  describe "GET /metrics with protobuf Accept" do
    it "returns protobuf content type and decodable body" do
      ...
      expect(response.status).to be(:==, 200)
      expect(response.headers["content-type"]).to be(:==, Fast::Prometheus::Formats::Protobuf::CONTENT_TYPE)
      body = response.read
