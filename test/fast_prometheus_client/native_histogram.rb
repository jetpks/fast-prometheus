...
describe FastPrometheusClient::NativeHistogram do
  describe ".new" do
    ...
    it "raises on schema 9" do
      expect do
        FastPrometheusClient::NativeHistogram.new(:t, docstring: "t", schema: 9)
      end.to raise_exception(ArgumentError)
    end

    it "raises on schema -5" do
      expect do
        FastPrometheusClient::NativeHistogram.new(:t, docstring: "t", schema: -5)
      end.to raise_exception(ArgumentError)
    end
  end
  ...
  describe "index_for" do
    it "anchors: schema 0, 4.0 -> 2" do
      nh = FastPrometheusClient::NativeHistogram.new(:t, docstring: "t", schema: 0)
      nh.observe(4.0)
      slot = nh.get
    ...
    it "anchors: schema 3, 1.0 -> 0" do
      nh = FastPrometheusClient::NativeHistogram.new(:t, docstring: "t", schema: 3)
      nh.observe(1.0)
      slot = nh.get
      expect(slot.positive_buckets).to be(:==, [[0, 1]])
    end
  ...
  describe "downscale" do
    it "respects max_buckets" do
      nh = FastPrometheusClient::NativeHistogram.new(:t, docstring: "t", max_buckets: 4)
